...
public class Ili9488Driver {
     ...
     * @throws IOException
     */
    public void writeEveryPixel16Bit(int redByte, int greenByte, int blueByte) throws IOException {
        //sendData(redByte);
        //sendData(greenByte);
        //sendData(blueByte);
        DC.high();
        //CS.low();
        spi.write((byte) redByte);
        spi.write((byte) greenByte);
        spi.write((byte) blueByte);
    }
    ...
    public void drawImg16BitColor(BufferedImage bufferedImage) throws IOException {
        ...
        for (int y = HEIGHT - 1; y >= 0; y--) {
            for (int x = 0; x < WIDTH; x++) {
                long countstart = new Date().getTime();
                ...
                colorBytes[i] = blue;
                i++;
                counttime += (new Date().getTime() - countstart);
            }
            i=0;
            long writestart = new Date().getTime();
            Spi.wiringPiSPIDataRW(0, colorBytes, colorBytes.length);
            writeTime += (new Date().getTime() - writestart);
        }

        LOG.debug("countTime = " + counttime);
        LOG.debug("writeTime = " + writeTime);
